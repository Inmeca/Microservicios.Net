...
namespace Example.Catalogos.Test.Services.Catalogos
{
    ...
    public class CatalogosServiceTest : BaseTest
    {
        ...
        {
            var result = await this.modelServices.GetCatalogosAsync(10);

            Assert.True(result != null);
            Assert.True(result.FirstName == "Augusto");
        }
        ...
        {
            // Arrange
            var model = this.UpdateCatalogosDto();

            // Act
            var result = await this.modelServices.UpdateCatalogos(model);
            var find_result = await this.modelServices.GetCatalogosAsync(1);

            // Assert
            Assert.IsNotNull(result);
            Assert.IsTrue(result);

            Assert.True(find_result.FirstName == "Ingrid");
        }
        ...
        /// <returns>A <see cref="Task"/> representing the asynchronous unit test.</returns>
        [Test]
        public async Task DeleteCatalogos()
        {
            var result = await this.modelServices.DeleteCatalogos(2);
            var find_result = await this.modelServices.GetCatalogosAsync(2);

            // Assert
            Assert.IsNotNull(result);
            Assert.IsTrue(result);

            Assert.True(find_result == null);
        }
    }
}
